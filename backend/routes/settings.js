...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      const getEmail = await myDB.query(
        "SELECT email FROM public_users WHERE id = $1",
        [req.user.id],
      );
...
router.patch("/api/reset-password", checkAuth, async (req, res, next) => {
  const userId = req.user.id;
